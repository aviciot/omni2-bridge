    ...
    try:
        while True:
            ...
            try:
                ...
                if prompt_guard:
                    try:
                        # Load config to check bypass_roles
                        guard_config = None
                        ...
                        if not guard_config:
                            ...
                            guard_config = {
                                ...
                                "behavioral_tracking": {"enabled": True, "warning_threshold": 3, "block_threshold": 5}
                            }
                        ...
                        if guard_enabled and context.get('role_name') not in bypass_roles:
                            ...
                            if not guard_result["safe"]:
                                logger.warning(f"[WS-CHAT] 🚨 INJECTION DETECTED! Score: {guard_result['score']}, Action: {guard_result['action']}")
                                
                                # Publish violation event to Redis for dashboard notifications
                                ...
                                if redis_client:
                                    ...
                                    logger.info(f"[WS-CHAT] 📡 Published notification to system_events: {notification_data['type']}")
                                else:
                                    logger.warning("[WS-CHAT] ⚠️ Redis client not available for notifications")
                                
                                # Handle actions based on guard result
                                action = guard_result["action"]
                                score = guard_result["score"]
                                
                                if action == "block":
                                    logger.warning(f"[WS-CHAT] 🚫 Message blocked (score: {score})")
                                    await websocket.send_json({
                                        "type": "blocked",
                                        "message": "Message blocked due to security policy violation",
                                    })
                                    continue
                                
                                elif action == "warn":
                                    logger.warning(f"[WS-CHAT] ⚠️ Injection warning (score: {score})")
                                    await websocket.send_json({
                                        "type": "warning",
                                        "message": "Suspicious content detected. Please review your message.",
                                    })
                                    # Continue to LLM (warn mode allows message through)
                            else:
                                logger.info(f"[WS-CHAT] ✅ Message passed guard check (score: {guard_result['score']})")
                        else:
                            if not guard_enabled:
                                logger.info("[WS-CHAT] 🛡️ Guard disabled, skipping check")
                            else:
                                logger.info(f"[WS-CHAT] 🛡️ Role '{context.get('role_name')}' bypassed guard check")
                    
                    except Exception as e:
                        logger.error(f"[WS-CHAT] ❌ Prompt guard error: {e}")
                        # Continue processing on guard error (fail open)
                else:
                    logger.warning("[WS-CHAT] ⚠️ Prompt guard client not available")
