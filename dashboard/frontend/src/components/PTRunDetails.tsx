...
export default function PTRunDetails({ runId, onClose }: PTRunDetailsProps) {
  ...
  const [agentStories, setAgentStories] = useState<any[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      setAgentStoryCount(stories.length);
  ...
  const downloadReport = () => {
    const d = runData;
    if (!d) return;
    const ts = (s: string) => new Date(s).toLocaleString();
    const hr = (ch = "─") => ch.repeat(60);
    const lines: string[] = [];

    lines.push(`# MCP Penetration Test Report`);
    lines.push(`\n${hr()}`);
    lines.push(`**Run #${runId}** | ${d.mcp_name} | Preset: ${d.preset}`);
    lines.push(`Status: **${d.status?.toUpperCase()}** | Started: ${ts(d.created_at)}${d.completed_at ? ` | Finished: ${ts(d.completed_at)}` : ""}`);
    if (d.llm_provider && d.llm_provider !== "template") lines.push(`LLM: ${d.llm_provider} / ${d.llm_model}${d.llm_cost_usd > 0 ? ` ($${Number(d.llm_cost_usd).toFixed(4)})` : ""}`);
    if (d.pt_score != null) lines.push(`PT Score: **${d.pt_score}/100** (${d.pt_status})`);
    lines.push(``);

    lines.push(`## Test Results Summary\n`);
    lines.push(`| Metric | Count |`);
    lines.push(`|--------|-------|`);
    lines.push(`| Total Tests | ${d.total_tests ?? 0} |`);
    lines.push(`| Passed | ${d.passed ?? 0} |`);
    lines.push(`| Failed | ${d.failed ?? 0} |`);
    lines.push(`| Errors | ${d.errors ?? 0} |`);
    lines.push(`| Critical Findings | ${d.critical ?? 0} |`);
    lines.push(`| High Findings | ${d.high ?? 0} |`);
    lines.push(`| Medium Findings | ${d.medium ?? 0} |`);
    lines.push(`| Low Findings | ${d.low ?? 0} |`);
    lines.push(``);
    ...
    if (failed.length > 0) {
      ...
      failed.forEach(r => {
        lines.push(`### [${(r.severity ?? "info").toUpperCase()}] ${r.test_name}`);
    ...
    if (errors.length > 0) {
      lines.push(`## Test Errors (${errors.length})\n`);
      errors.forEach(r => {
        lines.push(`- **${r.test_name}**: ${r.error_message ?? "unknown error"}`);
      });
      lines.push(``);
    }

    if (agentStories.length > 0) {
      lines.push(`## AI Red Team Findings (${agentStories.length} scenarios)\n`);
      agentStories.forEach((s: any) => {
        const verdict = s.verdict === "vulnerability_found" ? "🔴 VULNERABILITY FOUND" : s.verdict === "secure" ? "🟢 SECURE" : "🟡 INCONCLUSIVE";
        lines.push(`### Scenario ${s.story_index}: ${s.title || s.attack_goal}`);
        lines.push(`**Verdict**: ${verdict}${s.verdict === "vulnerability_found" ? ` — ${(s.severity ?? "").toUpperCase()}` : ""}`);
        lines.push(`**Goal**: ${s.attack_goal}`);
        if (s.finding) lines.push(`**Finding**: ${s.finding}`);
        if (s.evidence) lines.push(`**Evidence**: ${s.evidence}`);
        if (s.recommendation && s.verdict === "vulnerability_found") lines.push(`**Recommendation**: ${s.recommendation}`);
        if (s.was_planned === false) lines.push(`> ⚡ Unexpected finding — not in pre-assigned scenarios`);
        lines.push(``);
      });
    }

    lines.push(`${hr()}`);
    lines.push(`_Generated by OMNI2 MCP Penetration Testing — ${new Date().toLocaleString()}_`);

    const blob = new Blob([lines.join("\n")], { type: "text/markdown;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `pt-report-${d.mcp_name}-run${runId}-${new Date().toISOString().slice(0, 10)}.md`;
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={!runData || loading}
                title="Download Markdown report"
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-lg transition-colors"
              >
                📥 Download Report
              </button>
              <button
